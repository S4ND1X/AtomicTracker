...
class HabitListFragment : Fragment(), View.OnClickListener {
        ...
        Firebase.firestore.collection("habitos")
            .get().addOnSuccessListener {
                ...
                for(habit in it){
                    ...
                    var id = habit.id.toString()
                    var ubicacion = habit.get("Ubicacion").toString()
                    var currentHabit = Home.habit(id,nombre, descripcion, frecuencia, inicio, fin, ubicacion)
    ...
    override fun onClick(p0: View) {
        ...
        //Toast.makeText(this.context, "MY ID IS : ${actual.id}", Toast.LENGTH_SHORT).show()
        habitFragment = HabitFragment.newInstance(actual.id, actual.nombre, actual.descripcion, actual.inicio, actual.fin, actual.frecuencia, actual.ubicacion)
